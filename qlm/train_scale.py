...
def main():
    ...
    ap.add_argument("--reproject_every", type=int, default=200,
                    help="every N optimizer steps, snap Kr/Ki back onto the per-token "
                         "isometry manifold. The forward _KrausProjection already "
                         "projects every token every forward pass, so the MODEL is "
                         "always on-manifold; this only re-normalises the stored "
                         "parametrisation to prevent slow numerical drift in storage. "
                         "Cheap hygiene, NOT needed every step (a full-vocab batched "
                         "eigh is expensive). 0=off")
    ...
    try:
        for step in range(start_step, args.steps):
            ...
            if args.reproject_every and (step + 1) % args.reproject_every == 0:
                # Per-token conditioning telemetry: G is (V, n, n), one per token.
                # Report the WORST per-token conditioning across the vocab (the
                # global reshape would just give V*I -> eigenvalues == V, which is
                # meaningless). Then re-normalise the stored parametrisation.
                with torch.no_grad():
                    Vt = torch.complex(model.Kr, model.Ki).reshape(
                        model.Kr.shape[0], -1, model.n)         # (V, W*n, n)
                    G = Vt.mH @ Vt                              # (V, n, n)
                    s = torch.linalg.eigvalsh(G).real           # (V, n)
                    g_min = s.min().item(); g_max = s.max().item()
                model.reproject_()
                cond = g_max / max(g_min, 1e-12)
                if cond > 10.0:
                    print(f"   [reproject] step {step+1}: worst per-token G eigs "
                          f"[{g_min:.3e}, {g_max:.3e}] cond {cond:.1f} -> renormalised")
